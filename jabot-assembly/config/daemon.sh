...
CLASSPATH=".";
for I in /opt/jabot/lib/*.jar; do
    CLASSPATH="$CLASSPATH:$I"
done

serviceNameLo="jabot"                                  # service name with the first letter in lowercase
serviceName="jabot"                                    # service name
...
serviceLogFile="/opt/jabot/logs2/$serviceNameLo.log"               # log file for StdOut/StdErr
maxShutdownTime=15                                         # maximum number of seconds to wait for the daemon to terminate normally
pidFile="/opt/jabot/$serviceNameLo.pid"                      # name of PID file (PID = process ID number)
javaCommand="java"                                         # name of the Java launcher without the path
javaExe="$JAVA_HOME/bin/$javaCommand"                      # file name of the Java application launcher executable
javaArgs="-Xmx100m -server -cp $CLASSPATH jabot.App /opt/jabot/jabott.properties"                     # arguments for Java launcher
javaCommandLine="$javaExe $javaArgs"                       # command line to start the Java service application
javaCommandLineKeyword="jabot.App"                     # a keyword that occurs on the commandline, used to detect an already running service process and to distinguish it from others
